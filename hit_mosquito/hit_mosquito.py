...
def main(): #遊戲的主函數，負責初始化 pygame、創建窗口、設置遊戲循環等。
    """
    遊戲的主函數
    """
    ...
    # 設置一個定時事件，用於重新生成蚊子
    reload_mosquito_event = USEREVENT + 1   #設置自定義事件，用於定時重新生成蚊子。
    pygame.time.set_timer(reload_mosquito_event, 300)
    #設置定時器，300 毫秒後觸發 reload_mosquito_event 事件。

    points = 0  # 初始分數，初始化分數為 0。
    # 創建字體物件，用於顯示分數和打中提示
    my_font = pygame.font.SysFont(None, 30) #創建字體物件，用於顯示分數。
    my_hit_font = pygame.font.SysFont(None, 40) #創建字體物件，用於顯示打中提示。
    hit_text_surface = None
    main_clock = pygame.time.Clock()

    while True: #遊戲主循環，持續執行遊戲邏輯。
        # 事件處理
        for event in pygame.event.get():    #處理所有事件，如窗口關閉、鼠標點擊等。
            if event.type == QUIT:
                # 當用戶關閉視窗時退出遊戲
                pygame.quit()   #若事件類型為 QUIT，則退出遊戲。
                sys.exit()
        ...
        # 在窗口上繪製蚊子和分數
        window_surface.blit(mosquito.image, mosquito.rect)  #在窗口上繪製蚊子圖片。
        window_surface.blit(text_surface, (10, 0))  #在窗口上繪製分數文字。
